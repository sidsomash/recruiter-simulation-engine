...
**Important — must be run from `main`:** Initialize automatically creates (or checks out) a
dedicated local `candidate/<your-name>` branch before writing any of your data, and refuses to
run if you're on any other branch (see [Candidate Data & Git Branches](#-candidate-data--git-branches)
...
- **Branch enforcement:** Before writing any files, a deterministic script (`ensure_candidate_branch.py`, stdlib-only) creates/checks out a dedicated `candidate/<slug>` branch off `main` — see [Candidate Data & Git Branches](#-candidate-data--git-branches). Run this skill from `main`; it refuses to run on any other branch.
...
A: The branch guard (`ensure_candidate_branch.py`) blocks Initialize from running anywhere
except `main`, to keep candidate data off shared/feature branches. Two common causes:
- **"not on `main`"** → run `git checkout main`, then re-run `/initialize`.
