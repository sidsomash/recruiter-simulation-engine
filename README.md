...
**Important:** Initialize creates (or checks out) a dedicated local `candidate/<slug>` branch
before writing any of your data. For safety, it refuses to run from feature branches or the
*wrong* candidate branch — start from `main` unless you're already on your own
`candidate/<slug>` branch (see [Candidate Data & Git Branches](#-candidate-data--git-branches)
below). If you're mid-way through unrelated work on another branch, switch to `main` first:
`git checkout main`.
...
Ranking is **deterministic** — it runs a standalone Python script (`run_ranking.py`, standard library only, no venv required) and persists a ranked CSV to `assets/ranking_results.csv` (overwritten on each run) rather than recomputing scores via the model.
...
### **Candidate Branch**
The dedicated local git branch (`candidate/<slug>`) that holds one candidate's full data
history — resume, profile, preferences, simulations, and tailored résumés. Created and checked
out automatically by the Initialize skill (never on `main`). See
[Candidate Data & Git Branches](#-candidate-data--git-branches) below for the full workflow.

---

## 🌿 Candidate Data & Git Branches

**`main` is a clean, generic template — it never holds real candidate PII.** All candidate
data (résumé, profile, preferences, simulations, tailored résumés) lives on a dedicated local
branch per candidate: `candidate/<slug>` (e.g., `candidate/jane-doe`), branched off `main`.

### **Why**
- Keeps `main` safe to branch skill-development work from (new skills, bug fixes, roadmap
  items) without ever carrying someone's name, email, phone, or résumé along for the ride.
- Supports multiple candidate profiles in the same repository — each candidate gets their own
  isolated branch and history, instead of one shared set of files that gets overwritten.
- Makes it obvious, from `git branch`, which branches are safe to open PRs from (anything off
...
2. All of Initialize's file writes (and every later Simulation / Resume-Restructure run for
   that candidate) then happen on `candidate/jane-doe`, not `main`.
3. To work on a **different** candidate, switch back to `main` (`git checkout main`) and run
   `/initialize` again with the new candidate's resume — a second `candidate/<slug>` branch is
   created automatically.
4. To resume work on an **existing** candidate, just check out their branch directly
   (`git checkout candidate/jane-doe`) — no need to re-run Initialize.

### **Publishing is always your decision**
These branches are **local-only by default** — the skill never runs `git push`, opens a PR, or
sets up branch tracking. If you want to back up or share a candidate's branch, push it
yourself: `git push -u origin candidate/jane-doe`. Most users will keep candidate branches
local-only, since they contain personal data.
...
- **File sync:** Files are written once to the canonical `.github` location, then mirrored byte-for-byte into `.claude`/`.gemini` by a deterministic script (`sync_candidate_files.py`) — not regenerated by the model three times
- **Branch enforcement:** Before writing any files, a deterministic script (`ensure_candidate_branch.py`, stdlib-only) creates/checks out a dedicated `candidate/<slug>` branch off `main` — see [Candidate Data & Git Branches](#-candidate-data--git-branches). Run this skill from `main` (or from that candidate's own branch, which is a no-op); it refuses on any other branch.
...
- **Output:** Persisted CSV at `assets/ranking_results.csv` (overwritten each run), generated by the deterministic `run_ranking.py` script
- **Requires:** Python 3.8+ on PATH (`python3` or `python`) — no packages or virtual environment needed, script uses only the standard library
...
- Git installed and this repository initialized as a git repo — required by the Initialize skill's branch guard (`ensure_candidate_branch.py`), which creates/checks out a per-candidate `candidate/<slug>` branch before writing any candidate data
- Authentication will be prompted in Step 2
- Python 3.8+ available on PATH as `python3` or `python` — required by the Ranking skill (`run_ranking.py`) and by the Initialize skill's branch-guard (`ensure_candidate_branch.py`) and file-sync (`sync_candidate_files.py`) steps, no packages or virtual environment needed
...
Engine: Creates/checks out `candidate/<your-name>` branch, guides you through
        3 interactive prompts, and creates candidate_resume.md,
        candidate_profile.md, candidate_preferences.md on that branch.
```
...
Engine: Runs run_ranking.py, writes assets/ranking_results.csv, and reports:
        Ranking persisted to assets/ranking_results.csv
...
1. **From `main`, initialize your profile once** → Creates/checks out your `candidate/<slug>` branch and candidate files on it
...
6. **Working with a different candidate?** → `git checkout main` → re-run Initialize with their resume → new `candidate/<slug>` branch created automatically
...
A: See `references/ranking_rules.md` for the exact formula, applied deterministically by `run_ranking.py`. It's a weighted composite:
- Recruiter Screen Likelihood (45%)
- Interview Likelihood (20%)
- Degree Alignment (15%)
- Skill Alignment (10%)
- Experience Alignment (5%)
- Final Fit Summary (5%)
- Minus preference-violation penalties (severity-tiered: minor/moderate/major/clearance)

Edit `ranking_rules.md` to adjust weights if needed — `run_ranking.py` implements whatever this file defines.
...
A: Yes. Each candidate gets their own isolated local branch (`candidate/<slug>`), created
automatically by Initialize. To switch candidates: `git checkout main`, then re-run
`/initialize` with the new candidate's resume — a fresh `candidate/<slug>` branch is created.
To resume an existing candidate, just `git checkout candidate/<slug>` directly. See
[Candidate Data & Git Branches](#-candidate-data--git-branches).
...
### **Q: Initialize says "REFUSED" and won't run — what happened?**
A: The branch guard (`ensure_candidate_branch.py`) blocks Initialize from running on the wrong
branch, to keep candidate data off shared/feature branches. It's a no-op (not a refusal) if
you're already on the correct `candidate/<slug>` branch. Two common refusal causes:
- **"not on `main`"** → you're on a feature branch or a *different* candidate's branch; run
  `git checkout main`, then re-run `/initialize`.
- **"`main` has uncommitted changes"** → commit, stash, or discard them first, then re-run.
No candidate files are written until the guard passes.

### **Q: Should I commit my candidate files and simulations to Git?**
A: Yes — but only on your `candidate/<slug>` branch, never on `main`. Initialize creates that
branch for you automatically, and all candidate data (résumé, profile, preferences,
simulations, tailored résumés) should be committed there. Version-controlling them lets you:
...
Treat simulations like snapshots — preserve them. Candidate branches are local-only by
default; push them yourself only if you intend to share or back them up.
